...
def test_compute_marginals_naive():
    ...
    # Conditioning on an impossible event is undefined and must not silently
    # return a factor containing NaNs.
    factors = [Factor(var=[0], card=[2], val=[1.0, 0.0])]
    try:
        compute_marginals_naive(0, factors, {0: 1})
    except ValueError:
        pass
    else:
...
def test_compute_marginals_bp():
    ...
    factors = [Factor(var=[0], card=[2], val=[1.0, 0.0])]
    try:
        compute_marginals_bp([0], factors, {0: 1})
    except ValueError:
        pass
    else:
        raise AssertionError('Zero-probability evidence should raise ValueError.')
...
def test_map_eliminate():
    ...
    # MAP inference must not convert caller-owned factors to log-space in place.
    factors = load_factor_list_from_json(os.path.join('data', 'graph_small.json'))
    original_values = [factor.val.copy() for factor in factors]
    map_eliminate(factors, None)
    for factor, original in zip(factors, original_values):
        assert np.array_equal(factor.val, original), 'Input factors were modified.'

    factors = [Factor(var=[0], card=[2], val=[1.0, 0.0])]
    try:
        map_eliminate(factors, {0: 1})
    except ValueError:
        pass
    else:
        raise AssertionError('Zero-probability evidence should raise ValueError.')
