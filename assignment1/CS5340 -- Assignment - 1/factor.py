...
class Factor:
    ...
    def __eq__(self, other):
        ...
        if np.allclose(self.val, other.val[other_index]):
            if (self.val_argmax is None or len(self.val_argmax) == 0) and \
                    (other.val_argmax is None or len(other.val_argmax) == 0):
                return True
            else:
                other_val_argmax_reordered = [other.val_argmax[i] for i in other_index]
                return self.val_argmax == other_val_argmax_reordered
        else:
            return False
